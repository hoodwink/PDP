...
app.controller('MainCtrl', function ($scope, $http, uiGridConstants) {
    ...
    $scope.gridAnnotate = {
        ...
        exporterCsvFilename: 'ANNOTATE.csv',
		exporterPdfFilename: 'ANNOTATE.pdf',
        paginationPageSize: 500,
        //useExternalPagination: true,
       // useExternalSorting: true,

        columnDefs: [
			{ field: 'ANNOTATE', displayName: 'Annotated Information Code', width: 275},
			{ field: 'DESCRIP', displayName: 'Description'}
        ],
        exporterAllDataFn: function () {
            return getPage()
			.then(function () {
			    $scope.gridAnnotate.useExternalPagination = false;
			    $scope.gridAnnotate.useExternalSorting = false;
			    getPage = null;
			});
        },
    ...
    var getPage = function () {
        var url = "http://PdpApi/api/Annotate";
                
        $scope.loading = true;
        $scope.msg = "";
        return $http.get(url)
		.success(function (response) {
		    $scope.gridAnnotate.totalItems = response.RecordCount;
		    $scope.gridAnnotate.data = response; // data.slice(firstRow, firstRow + pageSize)
		}).error(function (data) {
