var app = angular.module('app', [ //appConfMethod
	'ui.grid',
	//'ui.grid.edit',
	'ui.grid.pagination',
	//'ui.grid.selection',
...
app.controller('MainCtrl', function ($scope, $http, uiGridConstants) {
    ...
    $scope.gridConfMethod = {
        enableHorizontalScrollbar: 0,
        enableFiltering: true,
        enableGridMenu: true,
        exporterMenuCsv: true,
        exporterCsvFilename: 'ConfMethod.csv',
		exporterPdfFilename: 'ConfMethod.pdf',
        paginationPageSize: 500,
        //useExternalPagination: true,
        // useExternalSorting: true,

        columnDefs: [
			{ field: 'CONFMETHOD', displayName: 'Confirmation Method code', width: 225 },
			{ field: 'DESCRIP', displayName: 'Description' }
        ],
        exporterAllDataFn: function () {
            return getPage()
			.then(function () {
			    $scope.gridConfMethod.useExternalPagination = false;
			    $scope.gridConfMethod.useExternalSorting = false;
			    getPage = null;
			});
        },
    ...
    var getPage = function () {
        var url = "http://PdpApi/api/ConfMethod";

        $scope.loading = true;
        $scope.msg = "";
        return $http.get(url)
		.success(function (response) {
		    $scope.gridConfMethod.totalItems = response.RecordCount;
		    $scope.gridConfMethod.data = response; // data.slice(firstRow, firstRow + pageSize)
		}).error(function (data) {
