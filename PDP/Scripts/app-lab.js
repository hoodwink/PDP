var app = angular.module('app', [//appLab
	'ui.grid',
	//'ui.grid.edit',
	'ui.grid.pagination',
	//'ui.grid.selection',
...
app.controller('MainCtrl', function ($scope, $http, uiGridConstants) {
    ...
    $scope.gridLab = {
        ...
        exporterCsvFilename: 'Lab.csv',
        exporterPdfFilename: 'Lab.pdf',
        paginationPageSize: 500,
        //useExternalPagination: true,
        // useExternalSorting: true,

        columnDefs: [
			{ field: 'LAB', displayName: 'Laboratory code', width: 225 },
			{ field: 'DESCRIP', displayName: 'Description' }
        ],
        exporterAllDataFn: function () {
            return getPage()
			.then(function () {
			    $scope.gridLab.useExternalPagination = false;
			    $scope.gridLab.useExternalSorting = false;
			    getPage = null;
			});
        },
    ...
    var getPage = function () {
        var url = "http://PdpApi/api/Lab";

        $scope.loading = true;
        $scope.msg = "";
        return $http.get(url)
		.success(function (response) {
		    $scope.gridLab.totalItems = response.RecordCount;
		    $scope.gridLab.data = response; // data.slice(firstRow, firstRow + pageSize)
		}).error(function (data) {
