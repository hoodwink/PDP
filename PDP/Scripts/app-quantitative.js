var app = angular.module('app', [ //appquantative
	'ui.grid',
	//'ui.grid.edit',
	'ui.grid.pagination',
	//'ui.grid.selection',
...
app.controller('MainCtrl', function ($scope, $http, uiGridConstants) {
    ...
    $scope.gridQuantative = {
        enableHorizontalScrollbar: 0,
        enableFiltering: true,
        enableGridMenu: true,
        exporterMenuCsv: true,
        exporterCsvFilename: 'Quantative.csv',
        exporterPdfFilename: 'Quantative.pdf',
        paginationPageSize: 500,
        //useExternalPagination: true,
        // useExternalSorting: true,

        columnDefs: [
			{ field: 'QUANTITATE', displayName: 'Quantitation Method code', width: 225 },
			{ field: 'DESCRIP', displayName: 'Description' },
        ],
        exporterAllDataFn: function () {
            return getPage()
			.then(function () {
			    $scope.gridQuantative.useExternalPagination = false;
			    $scope.gridQuantative.useExternalSorting = false;
			    getPage = null;
			});
        },
    ...
    var getPage = function () {
        var url = "http://PdpApi/api/Quantitate";

        $scope.loading = true;
        $scope.msg = "";
        return $http.get(url)
		.success(function (response) {
		    $scope.gridQuantative.totalItems = response.RecordCount;
		    $scope.gridQuantative.data = response; // data.slice(firstRow, firstRow + pageSize)
		}).error(function (data) {
