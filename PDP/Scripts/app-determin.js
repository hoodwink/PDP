var app = angular.module('app', [//appDetermin
	'ui.grid',
	//'ui.grid.edit',
	'ui.grid.pagination',
	//'ui.grid.selection',
...
app.controller('MainCtrl', function ($scope, $http, uiGridConstants) {
    ...
    $scope.gridDetermin = {
        ...
        exporterCsvFilename: 'Determin.csv',
        exporterPdfFilename: 'Determin.pdf',
        paginationPageSize: 500,
        //useExternalPagination: true,
        // useExternalSorting: true,

        columnDefs: [
			{ field: 'DETERMIN', displayName: 'Determinative Method code', width: 225 },
			{ field: 'DESCRIP', displayName: 'Description' }
        ],
        exporterAllDataFn: function () {
            return getPage()
			.then(function () {
			    $scope.gridDetermin.useExternalPagination = false;
			    $scope.gridDetermin.useExternalSorting = false;
			    getPage = null;
			});
        },
    ...
    var getPage = function () {
        var url = "http://PdpApi/api/Determin";

        $scope.loading = true;
        $scope.msg = "";
        return $http.get(url)
		.success(function (response) {
		    $scope.gridDetermin.totalItems = response.RecordCount;
		    $scope.gridDetermin.data = response; // data.slice(firstRow, firstRow + pageSize)
		}).error(function (data) {
