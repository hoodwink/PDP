var app = angular.module('app', [ //appPesticide
	'ui.grid',
	//'ui.grid.edit',
	'ui.grid.pagination',
	//'ui.grid.selection',
...
app.controller('MainCtrl', function ($scope, $http, uiGridConstants) {
    ...
    $scope.gridPesticide = {
        ...
        exporterCsvFilename: 'Pesticide.csv',
        exporterPdfFilename: 'Pesticide.pdf',
        paginationPageSize: 500,
        //useExternalPagination: true,
        // useExternalSorting: true,

        columnDefs: [
			{ field: 'PESTCODE', displayName: 'Pesticide code', width: 225 },
			{ field: 'DESCRIP', displayName: 'Description' },
            { field: 'TESTCLASS', displayName: 'Test Class' }
        ],
        exporterAllDataFn: function () {
            return getPage()
			.then(function () {
			    $scope.gridPesticide.useExternalPagination = false;
			    $scope.gridPesticide.useExternalSorting = false;
			    getPage = null;
			});
        },
    ...
    var getPage = function () {
        var url = "http://PdpApi/api/Pesticide";

        $scope.loading = true;
        $scope.msg = "";
        return $http.get(url)
		.success(function (response) {
		    $scope.gridPesticide.totalItems = response.RecordCount;
		    $scope.gridPesticide.data = response; // data.slice(firstRow, firstRow + pageSize)
		}).error(function (data) {
