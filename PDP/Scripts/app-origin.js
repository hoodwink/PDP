var app = angular.module('app', [ //apporigin
	'ui.grid',
	//'ui.grid.edit',
	'ui.grid.pagination',
	//'ui.grid.selection',
...
app.controller('MainCtrl', function ($scope, $http, uiGridConstants) {
    ...
    $scope.gridOrigin = {
        ...
        exporterCsvFilename: 'Origin.csv',
		exporterPdfFilename: 'Origin.pdf',
        paginationPageSize: 500,
        //useExternalPagination: true,
        // useExternalSorting: true,

        columnDefs: [
			{ field: 'ORIGIN', displayName: 'Origin code', width: 225 },
			{ field: 'DESCRIP', displayName: 'Description' }
        ],
        exporterAllDataFn: function () {
            return getPage()
			.then(function () {
			    $scope.gridOrigin.useExternalPagination = false;
			    $scope.gridOrigin.useExternalSorting = false;
			    getPage = null;
			});
        },
    ...
    var getPage = function () {
        var url = "http://PdpApi/api/Origin";

        $scope.loading = true;
        $scope.msg = "";
        return $http.get(url)
		.success(function (response) {
		    $scope.gridOrigin.totalItems = response.RecordCount;
		    $scope.gridOrigin.data = response; // data.slice(firstRow, firstRow + pageSize)
		}).error(function (data) {
