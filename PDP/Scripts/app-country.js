var app = angular.module('app', [ //appcountry
	'ui.grid',
	//'ui.grid.edit',
	'ui.grid.pagination',
	//'ui.grid.selection',
...
app.controller('MainCtrl', function ($scope, $http, uiGridConstants) {
    ...
    $scope.gridCountry = {
		...
		exporterPdfFilename: 'Country.pdf',
        paginationPageSize: 500,
        //useExternalPagination: true,
        // useExternalSorting: true,

        columnDefs: [
			{ field: 'COUNTRY', displayName: 'Country of Origin code', width: 225 },
			{ field: 'DESCRIP', displayName: 'Description' }
        ],
        exporterAllDataFn: function () {
            return getPage()
			.then(function () {
			    $scope.gridCountry.useExternalPagination = false;
			    $scope.gridCountry.useExternalSorting = false;
			    getPage = null;
			});
        },
    ...
    var getPage = function () {
        var url = "http://PdpApi/api/Country";

        $scope.loading = true;
        $scope.msg = "";
        return $http.get(url)
		.success(function (response) {
		    $scope.gridCountry.totalItems = response.RecordCount;
		    $scope.gridCountry.data = response; // data.slice(firstRow, firstRow + pageSize)
		}).error(function (data) {
