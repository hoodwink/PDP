var app = angular.module('app', [ //appExtract
	'ui.grid',
	//'ui.grid.edit',
	'ui.grid.pagination',
	//'ui.grid.selection',
...
app.controller('MainCtrl', function ($scope, $http, uiGridConstants) {
    ...
    $scope.gridExtract = {
        ...
        exporterCsvFilename: 'Extract.csv',
        exporterPdfFilename: 'Extract.pdf',
        paginationPageSize: 500,
        //useExternalPagination: true,
        // useExternalSorting: true,

        columnDefs: [
			{ field: 'EXTRACT', displayName: 'Extraction Method code', width: 225 },
			{ field: 'DESCRIP', displayName: 'Description' }
        ],
        exporterAllDataFn: function () {
            return getPage()
			.then(function () {
			    $scope.gridExtract.useExternalPagination = false;
			    $scope.gridExtract.useExternalSorting = false;
			    getPage = null;
			});
        },
    ...
    var getPage = function () {
        var url = "http://PdpApi/api/Extract";

        $scope.loading = true;
        $scope.msg = "";
        return $http.get(url)
		.success(function (response) {
		    $scope.gridExtract.totalItems = response.RecordCount;
		    $scope.gridExtract.data = response; // data.slice(firstRow, firstRow + pageSize)
		}).error(function (data) {
