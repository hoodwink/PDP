var app = angular.module('app', [ //appState
	'ui.grid',
	//'ui.grid.edit',
	'ui.grid.pagination',
	//'ui.grid.selection',
...
app.controller('MainCtrl', function ($scope, $http, uiGridConstants) {
    ...
    $scope.gridState = {
        ...
        exporterCsvFilename: 'State.csv',
        exporterPdfFilename: 'State.pdf',
        paginationPageSize: 500,
        //useExternalPagination: true,
        // useExternalSorting: true,

        columnDefs: [
			{ field: 'STATE', displayName: 'State code', width: 225 },
			{ field: 'DESCRIP', displayName: 'Description' },
        ],
        exporterAllDataFn: function () {
            return getPage()
			.then(function () {
			    $scope.gridState.useExternalPagination = false;
			    $scope.gridState.useExternalSorting = false;
			    getPage = null;
			});
        },
    ...
    var getPage = function () {
        var url = "http://PdpApi/api/State";

        $scope.loading = true;
        $scope.msg = "";
        return $http.get(url)
		.success(function (response) {
		    $scope.gridState.totalItems = response.RecordCount;
		    $scope.gridState.data = response; // data.slice(firstRow, firstRow + pageSize)
		}).error(function (data) {
