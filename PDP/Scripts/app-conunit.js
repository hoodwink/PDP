var app = angular.module('app', [ //appconunit
	'ui.grid',
	//'ui.grid.edit',
	'ui.grid.pagination',
	//'ui.grid.selection',
...
app.controller('MainCtrl', function ($scope, $http, uiGridConstants) {
    ...
    $scope.gridConUnit = {
        ...
        exporterCsvFilename: 'Conunit.csv',
		exporterPdfFilename: 'Conunit.pdf',
        paginationPageSize: 500,
        //useExternalPagination: true,
        // useExternalSorting: true,

        columnDefs: [
			{ field: 'CONUNIT', displayName: 'Concentration/LOD Units code', width: 225 },
			{ field: 'DESCRIP', displayName: 'Description' }
        ],
        exporterAllDataFn: function () {
            return getPage()
			.then(function () {
			    $scope.gridConUnit.useExternalPagination = false;
			    $scope.gridConUnit.useExternalSorting = false;
			    getPage = null;
			});
        },
    ...
    var getPage = function () {
        var url = "http://PdpApi/api/ConUnit";

        $scope.loading = true;
        $scope.msg = "";
        return $http.get(url)
		.success(function (response) {
		    $scope.gridConUnit.totalItems = response.RecordCount;
		    $scope.gridConUnit.data = response; // data.slice(firstRow, firstRow + pageSize)
		}).error(function (data) {
