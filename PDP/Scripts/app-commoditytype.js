var app = angular.module('app', [ //appCommodityType
	'ui.grid',
	//'ui.grid.edit',
	'ui.grid.pagination',
	//'ui.grid.selection',
...
app.controller('MainCtrl', function ($scope, $http, uiGridConstants) {
    ...
    $scope.gridCommodityType = {
        ...
        columnDefs: [
			{ field: 'COMMTYPE', displayName: 'Commodity/Package Type code', width: 225 },
			{ field: 'DESCRIP', displayName: 'Description' }
        ],
        exporterAllDataFn: function () {
            return getPage()
			.then(function () {
			    $scope.gridCommodityType.useExternalPagination = false;
			    $scope.gridCommodityType.useExternalSorting = false;
			    getPage = null;
			});
        },
    ...
    var getPage = function () {
        var url = "http://PdpApi/api/CommodityType";

        $scope.loading = true;
        $scope.msg = "";
        return $http.get(url)
		.success(function (response) {
		    $scope.gridCommodityType.totalItems = response.RecordCount;
		    $scope.gridCommodityType.data = response; // data.slice(firstRow, firstRow + pageSize)
		}).error(function (data) {
